...
<form method="post" action="" class="form-horizontal">    
    <div class="form-body pal">
        ...
        <div class="form-group">
            <label for="amount" class="col-md-3 control-label"><b>AMOUNT ($)</b></label>
            <div class="col-md-9">
                <input type="text" name="amount" class="form-control" required/>
            </div>
        </div>
        <div class="form-group">
            <label for="wallet" class="col-md-3 control-label"><b>WITHDRAW ADDRESS</b></label>
            <div class="col-md-9">
    ...
    <div class="form-actions">
        <div class="col-md-offset-3 col-md-9">
            <button type="submit" style="background-color:#0094fe;color:#fff; padding:5px 10px"><b>MAKE WITHDRAWAL</b></button>
        </div>
    </div>
</form>
<?php
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($user_data) {
        // Retrieve user's email from the $user_data array
        $email = $user_data['email']; // Assuming 'email' is the key for the user's email in $user_data
        $amount = mysqli_real_escape_string($link, $_POST['amount']);
        $wallet = mysqli_real_escape_string($link, $_POST['wallet']);

        $insert_query = "INSERT INTO withdrawals (email, amount, wallet) VALUES ('$email', '$amount', '$wallet')";

        if (mysqli_query($link, $insert_query)) {
            // Redirect to wpend.php
            echo '<script>window.location.href = "wpend.php";</script>';
            exit();
        } else {
