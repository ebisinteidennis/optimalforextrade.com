...
<div class="form-actions">
    <div class="col-md-offset-3 col-md-9">
        <a href="withdrawforusers.php">
            <button type="button" class="btn">
                <i class="fa fa-plus-circle" aria-hidden="true"></i> New Withdraw
            </button>
        </a>
    </div>
</div>
...
if ($user_data) {
    ...
    // Display all transactions for the logged-in user based on their ID
    $withdrawal_query = "SELECT id, date, amount, wallet, method, bank_name, acct_number, acct_name, acct_swift, status FROM withdrawals WHERE id = '$id'";
    $withdrawal_result = mysqli_query($link, $withdrawal_query);
    ...
    if ($withdrawal_result) {
        if (mysqli_num_rows($withdrawal_result) > 0) {
            ...
            echo '<th><center>METHOD</center></th>';
            echo '<th><center>BANK NAME</center></th>';
            echo '<th><center>ACCT NUMBER</center></th>';
            echo '<th><center>ACCT NAME</center></th>';
            echo '<th><center>ACCT SWIFT</center></th>';
            ...
            while ($row = mysqli_fetch_assoc($withdrawal_result)) {
                ...
                echo '<td><center>' . $row['wallet'] . '</center></td>';
                echo '<td><center>' . $row['method'] . '</center></td>';
                echo '<td><center>' . $row['bank_name'] . '</center></td>';
                echo '<td><center>' . $row['acct_number'] . '</center></td>';
                echo '<td><center>' . $row['acct_name'] . '</center></td>';
                echo '<td><center>' . $row['acct_swift'] . '</center></td>';
            ...
            echo '<p>No withdrawal history available.</p>';
        }
    } else {
        echo "Error fetching withdrawal history. Error details: " . mysqli_error($link);
    }
} else {
